...
from model.pytorch.loss import masked_mse_loss
...
class DCRNNSupervisor:
    ...
    def load_model(self, last_epoch=False):
        self._setup_graph()
        epoch_loaded=0

        # new addition by felix 
        if last_epoch:
            files=[f for f in os.listdir('models/') if os.path.isfile(os.path.join('models/', f))]
            fi=[int(f[3:-4]) for f in files if f.endswith('.tar')]
            epoch_loaded=max(fi)
            checkpoint = torch.load('models/epo%d.tar' % max(fi), map_location='cpu')
        else:
            assert os.path.exists('models/epo%d.tar' % self._epoch_num), 'Weights at epoch %d not found' % self._epoch_num
            checkpoint = torch.load('models/epo%d.tar' % self._epoch_num, map_location='cpu')
            epoch_loaded=self._epoch_num
        
        
        self.dcrnn_model.load_state_dict(checkpoint['model_state_dict'])
        self._logger.info("Loaded model at {}".format(epoch_loaded))
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                mse = self._compute_mse_loss(y, output)
            ...
            rmse_loss = np.sqrt(np.mean(mses))
            
            mean_std = np.std(losses)
            mape_std = np.std(mapes)
            rmse_std = np.std(np.sqrt(mses))


            self._writer.add_scalar('{} loss'.format(dataset), mean_loss, mape_loss, rmse_loss, batches_seen)
            ...
            return mean_loss, {'prediction': y_preds_scaled, 'truth': y_truths_scaled}, mape_loss, rmse_loss, mean_std, mape_std, rmse_std
    ...
    def _train(self, base_lr,
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            # restarting from last checkpoint if val loss increases too much
            # new addition by felix
            if val_loss >= min_val_loss*1.5:
                wait = 0
                self._logger.info('Val loss increase from {:.4f} to {:.4f}, '
                                  'restarting from last checkpoint'.format(min_val_loss, val_loss))
                self.load_model(last_epoch=True)
    ...
    def _compute_mse_loss(self, y_true, y_predicted):
        y_true = self.standard_scaler.inverse_transform(y_true)
        y_predicted = self.standard_scaler.inverse_transform(y_predicted)
